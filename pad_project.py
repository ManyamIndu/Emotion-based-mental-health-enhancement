...
def play_music(emotion):
    music_folder = f'mp3/{emotion}/'
    if os.path.exists(music_folder):
        songs = [f for f in os.listdir(music_folder) if f.endswith('.mp3')]
        if songs:
            song_path = os.path.join(music_folder, songs[0])  # Play the first song
...
face_cascade=cv2.CascadeClassifier("C:/Users/user/Downloads/app/haarcascade_frontalface_default.xml")
ds_factor=0.6
from tensorflow.keras import Input

emotion_model = Sequential()
emotion_model.add(Input(shape=(48, 48, 1)))  # Define input explicitly
...
class VideoCamera(object):
    def get_frame(self):
        ...
        for (x,y,w,h) in face_rects:
            ...
            stop_music()
            play_music(emotion_name)
            cv2.putText(image, emotion_name, (x+20, y-60), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
            df1 = music_rec()
        global last_frame1
